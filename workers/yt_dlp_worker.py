...
def friendly_error(error: Exception, cookie_file: Path | None, cookie_issue: str | None) -> str:
    ...
    if "403" in message or "Forbidden" in message:
        if cookie_file:
            return "YouTube returned 403 Forbidden. Update yt-dlp, then replace cookies.txt with a fresh Netscape-format export from the same browser account. Existing cookies may be expired."
        return "YouTube returned 403 Forbidden. Update yt-dlp and add a fresh Netscape-format cookies.txt export to the project cookies folder, then restart the Node server."
...
def options_for(args: argparse.Namespace, cookie_file: Path | None, output_directory: Path) -> dict[str, Any]:
    ...
    if args.format == "MP4":
        options["format"] = f"bestvideo[height<={args.quality}][ext=mp4]+bestaudio[ext=m4a]/best[height<={args.quality}][ext=mp4]/best[ext=mp4]/best"
    else:
        options["format"] = "bestaudio/best"
        options["postprocessors"] = [{"key": "FFmpegExtractAudio", "preferredcodec": AUDIO_CODECS[args.format], "preferredquality": args.quality}]
    return options
...
def progress_hook(status: dict[str, Any]) -> None:
    ...
    if status.get("status") == "downloading":
        ...
        emit({"event": "progress", "state": "downloading", "progress": round((downloaded / total) * 82, 1) if total else 0, "speed": status.get("speed"), "eta": status.get("eta")})
...
def main() -> int:
    ...
    emit({"event": "started", "cookies_configured": bool(cookie_file), "cookies_issue": cookie_issue})
    ...
    try:
        with yt_dlp.YoutubeDL(options_for(args, cookie_file, download_directory)) as downloader:
            ...
            if is_playlist:
                ...
                mimetype, filename = "application/zip", f"{title}.zip"
            else:
                output_path = Path(downloader.prepare_filename(metadata)).with_suffix(output_extension(args))
                if not output_path.is_file():
                    raise RuntimeError("yt-dlp completed without producing the expected output file.")
                mimetype, filename = None, output_path.name
        emit({"event": "complete", "path": str(output_path), "filename": filename, **({"mimetype": mimetype} if mimetype else {}), "archive": is_playlist, "title": title})
        return 0
